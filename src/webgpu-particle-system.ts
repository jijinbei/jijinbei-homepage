...
export class WebGPUParticleSystem {
  ...
  spawnParticlesFromFish(
    ...
    for (let i = 0; i < particleCount; i++) {
      // 魚の後ろの位置を計算
      const backwardDirection = fishVelocity.clone().normalize().multiplyScalar(-15);
      ...
      const particleColor = goldColors[Math.floor(Math.random() * goldColors.length)];
  ...
  update(): void {
    ...
    for (let i = 0; i < this.activeParticles; i++) {
      ...
      if (this.particleData.life[i] > 0) {
        ...
        this.particleData.position[i * 3 + 1] += this.particleData.velocity[i * 3 + 1];
        this.particleData.position[i * 3 + 2] += this.particleData.velocity[i * 3 + 2];
        ...
        // マトリックスを更新
        const lifeRatio = this.particleData.life[writeIndex] / this.particleData.maxLife[writeIndex];
        ...
        this.instancedMesh.setColorAt(writeIndex, new THREE.Color(
  ...
  private copyParticleData(from: number, to: number): void {
    ...
    this.particleData.position[to * 3 + 1] = this.particleData.position[from * 3 + 1];
    this.particleData.position[to * 3 + 2] = this.particleData.position[from * 3 + 2];
    
    // 速度
    this.particleData.velocity[to * 3] = this.particleData.velocity[from * 3];
    this.particleData.velocity[to * 3 + 1] = this.particleData.velocity[from * 3 + 1];
    this.particleData.velocity[to * 3 + 2] = this.particleData.velocity[from * 3 + 2];
